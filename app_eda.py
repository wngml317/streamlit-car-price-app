...
def run_eda() :
    ...
    st.write('{} 컬럼의 최대값에 해당하는 데이터'.format(selected_col))
    st.dataframe(df_max)
    st.write('{} 컬럼의 최소값에 해당하는 데이터'.format(selected_col))
    st.dataframe(df_min)
    ...
    if len(selected_list) > 1 :
        ...
        sb.heatmap(data=car_df[selected_list].corr(), annot=True, 
                    fmt='.2f', vmin=-1, vmax=1, cmap='coolwarm', linewidths=0.5)
        st.pyplot(fig2)
    ...
    # 1. 유저한테 검색어를 입력받는다
    word = st.text_input('이름을 검색할 단어를 검색하세요')
